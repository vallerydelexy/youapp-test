// define colors
import 'dart:ui';

class ColorApp {
  static const faintWhite = Color.fromRGBO(255, 255, 255, 0.06);
  static const hintWhite = Color.fromRGBO(255, 255, 255, 0.4);
  static const primary = Color.fromRGBO(98, 205, 203, 1);
  static const secondary = Color.fromRGBO(69, 153, 219, 1);
  static const List<Color> goldenGradientColorList = [
    Color.fromRGBO(148, 120, 62, 1),
    Color.fromRGBO(243, 237, 166, 1),
    Color.fromRGBO(248, 250, 229, 1),
    Color.fromRGBO(255, 226, 190, 1),
    Color.fromRGBO(213, 190, 136, 1),
    Color.fromRGBO(248, 250, 229, 1),
    Color.fromRGBO(213, 190, 136, 1),
  ];
}
